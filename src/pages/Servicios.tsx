                ...
                >
                  <AnimatePresence mode="wait">
                    {!isActive ? (
                      ...
                      >
                        <span className="font-serif text-2xl opacity-60 mr-4 lg:mr-0 lg:absolute lg:top-10">
                          {s.n}
                        </span>
                        <h3 className="font-serif text-2xl md:text-3xl lg:text-4xl whitespace-nowrap lg:-rotate-90 opacity-80 tracking-wide origin-center truncate lg:overflow-visible">
                          {s.title}
                        </h3>
                      </motion.div>
                    ) : (
                      ...
                      <motion.div 
                        ...
                        className="p-6 md:p-10 lg:p-12 flex flex-col w-full h-full lg:overflow-y-auto no-scrollbar"
                      >
                        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 w-full h-full">
                          
                          {/* Left: Title & Summaries */}
                          <div className="lg:w-1/2 flex flex-col gap-6 md:gap-8 flex-shrink-0">
                            <div className="flex items-baseline gap-4 md:gap-6">
                              <span className="font-serif text-3xl md:text-5xl opacity-50 italic">
                                {s.n}
                              </span>
                          ...
                          <div className="lg:w-1/2 flex flex-col gap-6 md:gap-8">
                            ...
                            <div className="flex flex-col gap-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {s.fit.map((f, j) => (
                                  <span 
                                    key={j} 
                                    className={`px-3 py-1.5 rounded-[2rem] text-[11px] md:text-xs tracking-wide font-medium border ${
                              ...
                              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4">
                                {s.includes.split(', ').map((item, j) => (
                                  <li key={j} className="flex items-start gap-2 border-b border-current/10 pb-1.5">
                                    ...
                                    <span className="text-xs md:text-sm font-medium capitalize leading-tight">{item}</span>
                                  </li>
                                ))}
                              </ul>
                            </div>
                            ...
                            {/* Result Block */}
                            <div className={`mt-auto p-5 md:p-6 rounded-[1rem] relative overflow-hidden ${
                              ...
                              <p className="font-serif italic text-lg md:text-xl leading-tight">
                                "{s.result}"
                              </p>
                            </div>
