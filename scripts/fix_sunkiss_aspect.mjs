...
async function main() {
  ...
  if (Array.isArray(blocks)) {
    blocks = blocks.map((b) => {
      if (b.type === 'image' && b.image?.publicId?.includes('ANTES_Y_DESPUES')) {
        return { ...b, aspect: 'auto' };
      }
      return b;
    });
    ...
    if (updateError) {
      console.error('Error updating sunkiss aspect in Supabase:', updateError);
    } else {
      console.log('Successfully updated sunkiss image aspect to auto in Supabase!');
    }
  }
}
